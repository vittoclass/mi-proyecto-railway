...
const SmartCameraModal = dynamic(() => import('../components/smart-camera-modal'), { 
    ssr: false, 
    loading: () => <div className="p-4">Cargando cámara...</div> 
});
...
export default function EvaluatorClient() {
    ...
    const onEvaluateAll = async () => { const { rubrica, pauta, flexibilidad, tipoEvaluacion } = form.getValues(); if (!rubrica) { form.setError("rubrica", { type: "manual", message: "La rúbrica es requerida." }); return; } setStudentGroups(groups => groups.map(g => g.files.length > 0 ? { ...g, isEvaluating: true, isEvaluated: false, error: undefined } : g)); for (const group of studentGroups) { if (group.files.length === 0) continue; const fileUrls = group.files.map(f => f.dataUrl); const payload = { fileUrls, rubrica, pauta, flexibilidad: flexibilidad[0], tipoEvaluacion }; const { jobId, error } = await startEvaluation(payload); if (error || !jobId) { setStudentGroups(prev => prev.map(g => g.id === group.id ? { ...g, isEvaluating: false, error: error || "No se pudo iniciar la tarea." } : g)); continue; } const intervalId = setInterval(async () => { const statusResponse = await checkEvaluationStatus(jobId); if (statusResponse.status === 'completed' || statusResponse.status === 'failed') { clearInterval(pollingIntervals.current.get(group.id)); pollingIntervals.current.delete(group.id); setStudentGroups(prev => prev.map(g => { if (g.id === group.id) { return { ...g, isEvaluating: false, isEvaluated: true, ...(statusResponse.status === 'completed' ? statusResponse.result : { error: statusResponse.error }) }; } return g; })); } }, 5000); pollingIntervals.current.set(group.id, intervalId); } };
    const generateReport = (group: StudentGroup) => { const { nombreProfesor, departamento, fechaEvaluacion, curso } = form.getValues(); const finalNota = (Number(group.nota) || 0) + group.decimasAdicionales; const reportWindow = window.open("", "_blank"); if (!reportWindow) return alert("Habilita las ventanas emergentes."); const reportHTML = `...`; reportWindow.document.write(reportHTML); reportWindow.document.close(); };

    // ===== INICIO DE LA CORRECCIÓN =====
    // Creamos una nueva variable para el estado de carga general.
