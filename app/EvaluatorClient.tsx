...
<svg viewBox="0 0 300 220" xmlns="http://www.w3.org/2000/svg" aria-label="Libel-IA logo">
  ...
  <rect x="149" y="166" width="2" height="14" 
rx="1" fill="#6366F1"/>
...
const ReportDocument = ({ group, formData, logoPreview }: any) => {
  const resumen = group.retroalimentacion?.resumen_general ||
{ fortalezas: 'N/A', areas_mejora: 'N/A' };
  ...
  return (
    <Document>
     ...
     <Text style={styles.infoText}>Evaluación: {formData.nombrePrueba || 'N/A'}</Text>
        ...
        <Text style={styles.studentLine}>Alumno: {group.studentName} · Curso: {formData.curso ||
'N/A'}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 6, marginTop: 6 }}>
          ...
          <View style={{ flex: 1, 
backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', padding: 5, borderRadius: 6, textAlign: 'center' as any }}>
...
gap: 8, marginTop: 8 }}>
          ...
          <View style={{ padding: 6, borderRadius: 6, flex: 1, backgroundColor: '#FFFBEB', borderWidth: 
1, borderColor: '#FDE68A' }}>
...
<View style={{ marginBottom: 6 }}>
            ...
            <View style={styles.table}>
              <View style={[styles.tableRow, { backgroundColor: '#F9FAFB' }]}>
                ...
                <View 
style={styles.tableColHeaderDetail}><Text style={styles.tableCellHeader}>Detalle</Text></View>
              </View>
...
<View style={styles.tableColDetail}><Text style={styles.tableCell}>{item.detalle}</Text></View>
   ...
   {group.retroalimentacion?.retroalimentacion_alternativas?.length > 0 && (
          <View style={{ marginBottom: 6 }}>
            ...
            <View style={styles.table}>
              <View style={[styles.tableRow, { backgroundColor: '#F9FAFB' }]}>
                <View style={styles.col40}><Text 
style={styles.tableCellHeader}>Pregunta</Text></View>
...
setUnassignedFiles(files => files.filter(f => f.id !== fileId));
  ...
  const removeFileFromGroup = (fileId: string, groupId: string) => {
    let fileToMoveBack: FilePreview |
undefined;
...
if (fileToMoveBack) setUnassignedFiles(prev => [...prev, fileToMoveBack!]);
  ...
  const handleDecimasChange = (groupId: string, value: string) => {
    const decimas = parseFloat(value) ||
0;
...
>
                  {({ loading }) => (
                    <Button size="sm" disabled={loading}>
                      {loading ?
'Preparando...' : <><Printer className="mr-2 h-4 w-4" /> Descargar PDF</>}
                    </Button>
                  )}
                </PDFDownloadLink>
      ...
      <main className="p-4 md:p-8 max-w-6xl mx-auto font-sans bg-[var(--bg-main)] text-[var(--text-primary)] transition-colors duration-300">
        ...
        <div className="flex items-center gap-2">
            <Button size="sm" variant={theme === 'theme-default' ?
'default' : 'ghost'} onClick={() => setTheme('theme-default')} className={cn(theme !== 'theme-default' && 'text-[var(--text-secondary)]')}>Predeterminado</Button>
            <Button size="sm" variant={theme === 'theme-ocaso' ?
'default' : 'ghost'} onClick={() => setTheme('theme-ocaso')} className={cn(theme !== 'theme-ocaso' && 'text-[var(--text-secondary)]')}>Ocaso</Button>
            <Button size="sm" variant={theme === 'theme-corporativo' ?
'default' : 'ghost'} onClick={() => setTheme('theme-corporativo')} className={cn(theme !== 'theme-corporativo' && 'text-[var(--text-secondary)]')}>Corporativo</Button>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4 bg-[var(--bg-muted)]">
            ...
            <TabsTrigger 
value="dashboard"><ClipboardList className="mr-2 h-4 w-4" />Resumen</TabsTrigger>
  ...
  <CardContent className="p-12">
                ...
                <p className="mt-6 text-lg text-[var(--text-secondary)]">Asistente pedagógico inteligente que analiza las 
respuestas de tus estudiantes, genera retroalimentación detallada y crea informes al instante.</p>
...
onEvaluateAll(); }} className="space-y-8">
       ...
       <SelectItem value="lenguaje">Lenguaje e Historia</SelectItem>
                    ...
                    <FormField control={form.control} name="flexibilidad" render={({ field }) => (
                     ...
                     <FormItem className="compact-field space-y-1">
                        ...
                        <FormControl>
                          <Slider 
min={1} max={5} step={1} defaultValue={field.value} onValueChange={field.onChange} />
                        </FormControl>
...
mb-4 text-[var(--text-accent)]">Personalización del Informe</h3>
  ...
  <Popover>
                              <PopoverTrigger asChild>
                                <FormControl>
                                  <Button 
variant={'outline'} className={cn('pl-3 text-left font-normal', !field.value && 'text-[var(--text-secondary)]')}>
                                    {field.value ?
format(field.value, 'PPP') : <span>Elige una fecha</span>}
   ...
   <div className="flex items-center gap-4">
                  ...
                  <FormField control={form.control} name="puntajeTotal" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>Ingresa el puntaje máximo de la evaluación.
Esto es obligatorio.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )} />
  ...
  <FormDescription>Describe los criterios de evaluación. Ya no es necesario incluir el puntaje total aquí.</FormDescription>
   ...
   <FormField control={form.control} name="pauta" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Textarea placeholder="Opcional.
Pega aquí las respuestas correctas..." className="min-h-[100px]" {...field} /></FormControl>
                      </FormItem>
                    )} />
                  </form>
                </Form>
...
className="self-center">
                        {isExtractingNames ?
<Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4 text-purple-500" />} Detectar Nombre
                      </Button>
                    </div>
                  </div>
                )}
...
text-[var(--text-accent)]">
    ...
    <button onClick={() => removeFileFromGroup(file.id, group.id)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5">
                 ...
                 <select onChange={(e) => { if (e.target.value) assignFileToGroup(e.target.value, group.id);
e.target.value = ''; }} className="text-sm bg-transparent">
                ...
                <CardFooter>
                  <Button size="lg" onClick={onEvaluateAll} disabled={isCurrentlyEvaluatingAny ||
studentGroups.every(g => g.files.length === 0)}>
                    {isLoading ||
isCurrentlyEvaluatingAny ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Evaluando...</> : <><Sparkles className="mr-2 h-4 w-4" /> Evaluar Todo</>}
                  </Button>
                </CardFooter>
              </Card>
            )}
                  ...
                  <div className="flex items-center gap-2">
                      ...
                      fileName={`informe_${group.studentName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`}
                       ...
                       <Button variant="ghost" size="sm" disabled={loading}>
                                    {loading ?
'Preparando PDF...' : <><Printer className="mr-2 h-4 w-4" /> Descargar PDF</>}
                                  </Button>
                                )}
...
Creada por un profesor, para profesores. Detecta respuestas, genera retroalimentación
                y entrega informes pedagógicos profesionales en segundos.
<b>1 crédito = 1 imagen.</b>
              ...
              <ul className="text-left space-y-2 mx-auto max-w-xl text-[var(--text-secondary)]">
                ...
                <li>✅ 
Informes PDF listos para imprimir o enviar.</li>
