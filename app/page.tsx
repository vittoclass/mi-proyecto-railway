...
export default function Page() {
  ...
  const handleFile = (file: File) => {
    ...
    form.reset({
        nombre: form.getValues("nombre"), // Mantenemos los valores que no dependen del archivo
        curso: form.getValues("curso"),
        rubrica: form.getValues("rubrica")
    });
  }
  ...
  const onSave = async () => {
    ...
    // El archivo ya fue subido en el paso de evaluación, reutilizamos la URL.
    const fileUrl = (await supabase.storage.from('imagenes').getPublicUrl(`evaluaciones/${fileToEvaluate!.name}`)).data.publicUrl;

    const finalData = { ...values, imagen: imagePreviewUrl, fecha: new Date().toISOString() };
